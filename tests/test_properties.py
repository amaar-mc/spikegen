...
means = st.floats(min_value=0.005, max_value=1.0, allow_nan=False, allow_infinity=False)
cvs = st.floats(min_value=0.05, max_value=3.0, allow_nan=False, allow_infinity=False)
...
@given(means, cvs, durations, seeds)
def test_lognormal_is_sorted_and_in_range(
    mean: float, cv: float, duration: float, seed: int
) -> None:
